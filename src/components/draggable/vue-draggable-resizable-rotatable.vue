<template>
  ...
  >
    <div
      ...
      :key="handle"
      :class="'handle-' + handle"
      :style="{ display: enabled ? 'block' : 'none', cursor: cursorStyle[handle]}"
...
export default {
  ...
  methods: {
    ...
    handleUp (e) {
      ...
      if (this.resizing) {
        ...
        this.rotating = false
        this.$emit('rotatestop', this.rotate, e)
      }
    ...
    fixedTo () {
      ...
      if (this.fixedXExchange) {
        fixedXName = this.fixedXName === 'right' ? 'left' : 'right'
      } else {
        fixedXName = this.fixedXName
      }
      if (this.fixedYExchange) {
        fixedYName = this.fixedYName === 'top' ? 'bottom' : 'top'
      } else {
        fixedYName = this.fixedYName
      }
  ...
  watch: {
    ...
    w (val) {
      this.width = val
      this.autoWidth = val === 'auto'
    },
    h (val) {
      this.height = val
      this.autoHeight = val === 'auto'
    },
    active (val) {
      this.enabled = val
    },
...
<style>
  ...
  .handle-rot {
    ...
    cursor: url(../../assets/rotate.svg) 20 20, move;
    z-index: -1;
  }
  .handle-rot:before {
    content: '';
