<template>
      ...
      <div class="controll">
        <div class="audio-state">
          ...
          <div class="btn play-button">
            <i
              class="iconfont"
              :class="{ 'icon-playfill': !playing, 'icon-stop': playing }"
              @click.stop="play"
            ></i>
          </div>
          <div class="btn">
          ...
          <div class="btn">{{duration}}</div>
          <div class="btn mute-button">
            <i
              class="iconfont"
              @click.stop="changeMute"
              :class="{'icon-notificationforbidfill': muted, 'icon-notificationfill': !muted, }"
            ></i>
          </div>
  ...
  >
    <audio
      ...
      preload
      autoplay
      :loop="loopType === loopTypes.single"
...
export default {
  ...
  mounted() {
    ...
    let w = Number(this.screen.w)
    let h = Number(this.screen.h)
    let data = {
      width: 500,
      height: 300,
      x: w>h ? 1152 - 510 : 720 - 510,
      y: w>h ? 720 - 310 : 1152 - 310,
      rotation: 0
    };
  ...
  computed: {
    ...
    curSongUrl() {
      let song = this.$store.state.music.songInfo.curSong;
      if (song) {
        if (song.url.indexOf("http://")==0) {
          //直接播放外链
          console.log("播放外链：",song.url)
          return song.url;
        }
  ...
  methods: {
    initWebsockt() {
      var ws = new WebSocket(`ws:/127.0.0.1:5432/?name=party3&bManage=false`);
      var that = this
    ...
    initCanvas() {
      ...
      if (playStyle != "nothing") {
        styles[playStyle].init(canvas, canvas.width / 800, canvas.height / 600);
      }
    },
    animate() {
      if (this.bManage) return;
    ...
    onActivated(e) {
      ...
      allFrame.forEach(frame => {
        // 禁止其它iframe的鼠标事件
        // if(frame.id != focusFrame.id){
        // console.log("禁止了ifram:", frame.id)
        frame.style =
          "overflow: hidden; width: 100%; height: 100%; border: 0px; backgournd:transparent;pointer-events:none; ";
        // }
      });
    },
    onDeactivated() {
      ...
      allFrame.forEach(frame => {
        ...
        frame.style =
          "overflow: hidden; width: 100%; height: 100%; border: 0px; backgournd:transparent;";
      });
    },
    openDrage(e) {
